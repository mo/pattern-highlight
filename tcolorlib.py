...
# webcolor can be "whitesmoke", "#00ff00" or "#ccc" etc
def webcolor_to_rgb(webcolor):
    webcolor = webcolor.strip().lower()
    if not webcolor.startswith("#"):
        webcolor = WEBCOLORS.get(webcolor, None)
        if not webcolor:
            return None
        return webcolor_to_rgb(webcolor)

    if len(webcolor) == 4:
        webcolor = "#" + webcolor[1] * 2 + webcolor[2] * 2 + webcolor[3] * 2
...
def rgb_to_ansi_256colors((r,g,b), background):
    ...
    if background:
        return "\033[48;5;%dm" % ansi256_color
    else:
        return "\033[38;5;%dm" % ansi256_color
